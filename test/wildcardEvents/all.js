...
if(typeof require !== 'undefined') {
  EventEmitter = require('../../lib/ChainedEventEmitter').EventEmitter;
}
else {
...
module.exports = basicEvents({
  ...
  '1. An event can be namespaced.': function (test) {
    ...
    emitter.emit('test1.ns1').then(function() {
      test.expect(1);
      test.done();
    });
  },
  '2. An event can be namespaced and accept values.': function (test) {
    ...
    emitter.emit('test2.ns1', 1).then(function() {
      test.expect(2);
      test.done();
    });
  ...
  '3. A namespaced event can be raised multiple times and accept values.': function (test) {
    ...
    emitter.emit('test3.ns1', 1, 2, 3).then(function() {
      emitter.emit('test3.ns1', 4, 5, 6).then(function() {
        test.expect(10);
        test.done();
      });
    });
  ...
  '4. A listener should support wild cards.': function (test) {
    ...
    emitter.emit('test4.ns1').then(function() {
      test.expect(1);
      test.done();
    });
  },
  '5. Emitting an event should support wildcards.': function (test) {
    ...
    emitter.emit('test5A.*').then(function() {
      test.expect(1);
      test.done();
    });
  ...
  '6. A listener should support complex wild cards.': function (test) {
    ...
    emitter.emit('test10.ns1.foo').then(function() {
      test.expect(1);
      test.done();
    });
  ...
  '7. Emitting an event should support complex wildcards.': function (test) {
    ...
    emitter.emit('test11.*.foo').then(function() {
      test.expect(1);
      test.done();
    });
  },
  '8. Emitting an event should support complex wildcards multiple times, a valid listener should accept values.': function (test) {
    ...
    emitter.emit('test12.*.ns2', 1, 2, 3).then(function() {
      emitter.emit('test12.*.ns2', 4, 5, 6).then(function() {
        test.expect(10);
        test.done();
      });
    });
  ...
  '10. should be able to listen on any event' : function (test) {
    ...
    emitter.onAny(fn);
    emitter.emit('test23.ns5.ns5', 'foo', 1).then(function() {
      test.expect(4);
      test.done();
    });
